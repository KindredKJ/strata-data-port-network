import {
  DirectPipeRuntime as __KindredDirectPipeRuntime,
  DIRECT_PIPE_CLASSIFICATIONS as __KindredDirectPipeClassifications,
  DIRECT_PIPE_LINEAGE as __KindredDirectPipeLineage
} from "../src/direct-pipe-runtime.v1.mjs";

import {
  DirectPipeProcessBoundaryAdapter as __KindredDirectPipeProcessBoundaryAdapter,
  DIRECT_PIPE_PROCESS_ATTACHMENT_VERSION as __KindredAttachmentVersion,
  DIRECT_PIPE_PROCESS_ATTACHMENT_CONTRACT as __KindredAttachmentContract
} from "../src/direct-pipe-process-boundary-adapter.v1.mjs";

import {
  writeFileSync as __kindredWriteFileSync
} from "node:fs";
...
test("Process Intelligence focused deterministic proof", async () => {
  ...
  assert.equal(boundary.mapPort("PROCESS"), "PROCESS_PORT");
    /* ==========================================================
       KINDRED LABS — SW06-D2-R2B
       DIRECT PIPE PROCESS ATTACHMENT PROOF
       Uses existing proven Process Intelligence fixture.
       ========================================================== */

    {
      const __kindredDirectPipeRuntime =
        new __KindredDirectPipeRuntime();

      const __kindredAttachment =
        new __KindredDirectPipeProcessBoundaryAdapter({
          processBoundary:
            boundary,

          directPipeRuntime:
            __kindredDirectPipeRuntime
        });

      const __kindredInput =
        Buffer.alloc(
          2 * 1024 * 1024,
          0x6b
        );

      const __kindredResult =
        __kindredAttachment.transferForProcessPort({
          port:
            ("PROCESS"),
      ...
      if (
        !Buffer.isBuffer(
          __kindredResult.output
        )
      ) {
        throw new Error(
          "SW06-D2-R2B: Direct Pipe output was not a Buffer."
        );
      }
      ...
      if (
        ...
        throw new Error(
          "SW06-D2-R2B: byte-integrity verification failed."
        );
      }

      if (
        __kindredResult.measurement.classification !==
        __KindredDirectPipeClassifications.PORTABLE_BUFFERED
      ) {
        throw new Error(
          "SW06-D2-R2B: unexpected Direct Pipe classification."
        );
      }

      if (
        __kindredResult.measurement.copyCount !==
        1
      ) {
        throw new Error(
          "SW06-D2-R2B: baseline must account for one explicit copy."
        );
      }

      if (
        __kindredResult.measurement.zeroCopyClaimAuthorized !==
        false
      ) {
        throw new Error(
          "SW06-D2-R2B: zero-copy claim unexpectedly authorized."
        );
      }

      const __kindredEvidencePath =
        process.env.KINDRED_DIRECT_PIPE_PROCESS_EVIDENCE;

      if (!__kindredEvidencePath) {
        throw new Error(
          "KINDRED_DIRECT_PIPE_PROCESS_EVIDENCE is required."
        );
      }

      __kindredWriteFileSync(
        __kindredEvidencePath,

        JSON.stringify(
          {
            schemaVersion:
              "kindred.direct-pipe.process-attachment-proof.v1",
            ...
            attachment: {
              contract:
                __KindredAttachmentContract,

              version:
                __KindredAttachmentVersion,
              ...
              fixtureSource:
                "process-intelligence.test.mjs",

              fixtureMode:
                "TRANSPLANTED_PROVEN_EXECUTION_CONTEXT"
            },

            lineage:
              __KindredDirectPipeLineage,

            measurement:
              __kindredResult.measurement,

            proof: {
              realProcessBoundaryFixture:
                true,

              realMapPortCallShape:
                true,

              realDirectPipeRuntime:
                true,
    ...
    /* END KINDRED SW06-D2-R2B ATTACHMENT PROOF */
